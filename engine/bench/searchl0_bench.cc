...
inline constexpr uint64_t NOW = 1000;

// Allocates a 32-byte aligned unit vector of size 384 along dimension 0.
// Ownership: Caller must std::free().
...
// Set a specified Arena slot READY with a normalized random vector
void SetReady(MemoryArena& arena, const size_t node_id, std::mt19937& rng) {
    ...
    arena.GetNode(node_id).control_block.store(
        PackControl(NodeState::READY, EvictState::HOT, 0, 0),
...
static void BenchSearchL0_AllReady(benchmark::State& state) {
    ...
    for ([[maybe_unused]] auto _ : state) {
        auto result = SearchL0(arena, query, NOW);
        benchmark::DoNotOptimize(result);
    }

    std::free(query);
    state.SetItemsProcessed(state.iterations() *
                            static_cast<int64_t>(L0_MAX_SLOTS));
}
