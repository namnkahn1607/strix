...
inline constexpr uint64_t kNow = 1000;  // fixed "current time" for all tests
inline constexpr uint64_t kFreshTS = kNow - 10;  // age 10s < lifespan (30s)
inline constexpr uint64_t kStaleTS = kNow - 60;  // age 60s > lifespan (30s)

// GenUnitVector(): allocates a 32-byte aligned float array of size 384, filled
// with a unit vector dimension `hot_dim`. Ownership: Caller must `std::free()`.
...
// GenRandomVector(): allocates a normalized random-ish 32-byte aligned float
// array of size 384. Again, ownership: Caller must `std::free()`.
...
// SetReady(): sets a specified Node READT with given vector.
void SetReady(MemoryArena& arena, const size_t node_id, const float* vec) {
    std::memcpy(arena.GetVector(node_id), vec, kVectorDim * sizeof(float));
    const uint64_t ctrl =
        PackControl(NodeState::kReady, EvictState::kHot, 0, 0);
    arena.GetNode(node_id).control_block.store(ctrl, std::memory_order_relaxed);
}

// SetPending(): sets a specified Node PENDING with given vector and timestamp.
...
TEST_F(SearchL0Test, EmptyArena) {
    ...
    const SearchResult res   = SearchL0(arena, query, kNow);
    std::free(query);
...
TEST_F(SearchL0Test, SingleReadyNode) {
    ...
    EXPECT_NEAR(res.best_score, 1.0f, kApprox4);
}
...
TEST_F(SearchL0Test, ClaimedSlotsAreSkipped) {
    ...
    std::memcpy(arena.GetVector(0), query, kVectorDim * sizeof(float));
...
TEST_F(SearchL0Test, FreshPendingIsSearchable) {
    ...
    SetPending(arena, 0, query, kFreshTS);
...
TEST_F(SearchL0Test, StalePendingIsSkipped) {
    float* query = GenUnitVector(3);
    SetPending(arena, 0, query, kStaleTS);
...
TEST_F(SearchL0Test, MigratingIsSearchable) {
    ...
    std::memcpy(arena.GetVector(0), query, kVectorDim * sizeof(float));
    arena.GetNode(0).control_block.store(
        PackControl(NodeState::kMigrating, EvictState::kHot, 0, 0),
        std::memory_order_relaxed);
...
TEST_F(SearchL0Test, SimilarityScoreIsAccurate) {
    ...
    EXPECT_NEAR(res.best_score, expected, kApprox4);
}
