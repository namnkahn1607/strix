// L0-tier node buffer implementation: ctor and register/removal methods.
...
NodeBuf::NodeBuf(const uint32_t capacity) : capacity{capacity} {
...
bool NodeBuf::TryPush(const uint32_t node_id) noexcept {
...
uint32_t NodeBuf::TryPop() noexcept {
