...
float DotProduct(float* query, float* node_vec) {
    float sum = 0.0f;
...
protected:
    void SetUp() override {
        query_      = AllocVecBuf(inference::kVectorMemsize);
        node_batch_ = AllocVecBuf(ann::kBatchSize * inference::kVectorMemsize);
    }
    ...
    void Refresh(uint64_t seed = 0x123) const noexcept {
        ...
        for (uint32_t b = 0; b < ann::kBatchSize; ++b) {
            float* node = node_batch_ + b * inference::kVectorDim;
            for (size_t i = 0; i < inference::kVectorDim; ++i) {
                node[i] = dist(gen);
            }
    ...
    // Computes dot product values using kernel's APIs.
    // The results of each API are written onto its corresponding buffer.
    void Compute(float* cont_scores, float* disc_scores) const noexcept {
        ann::DotProductContiguousBatch(query_, node_batch_, cont_scores);

        float* v0 = node_batch_;
        float* v1 = v0 + inference::kVectorDim;
        float* v2 = v1 + inference::kVectorDim;
        float* v3 = v2 + inference::kVectorDim;
        ann::DotProductDiscreteBatch(query_, v0, v1, v2, v3, disc_scores);
    }

    void GradeAgainstScalar(float* scores) const noexcept {
        for (uint32_t b = 0; b < ann::kBatchSize; ++b) {
            const float expected =
                DotProduct(query_, node_batch_ + b * inference::kVectorDim);
...
TEST_F(DotProductBatchTest, Deterministic) {
    ...
    float init_cont[ann::kBatchSize] = {};
    float init_disc[ann::kBatchSize] = {};
    Compute(init_cont, init_disc);

    for (uint32_t c = 0; c < kTotalCalls; ++c) {
        float curr_cont[ann::kBatchSize] = {};
        float curr_disc[ann::kBatchSize] = {};
        Compute(curr_cont, curr_disc);

        for (uint32_t b = 0; b < ann::kBatchSize; ++b) {
            EXPECT_EQ(curr_cont[b], init_cont[b])
                << "lane=" << b << "call=" << c;
            EXPECT_EQ(curr_disc[b], init_disc[b])
                << "lane=" << b << "call=" << c;

            // Cross-API deterministic
            EXPECT_NEAR(curr_cont[b], curr_disc[b], kApprox4)
                << "lane=" << b << "call=" << c;
        }
    }
}
...
TEST_F(DotProductBatchTest, ZeroQuery) {
    ...
    float cont_scores[ann::kBatchSize] = {};
    float disc_scores[ann::kBatchSize] = {};
    Compute(cont_scores, disc_scores);

    for (uint32_t b = 0; b < ann::kBatchSize; ++b) {
        EXPECT_FLOAT_EQ(cont_scores[b], 0.0f) << "lane=" << b;
...
TEST_F(DotProductBatchTest, CompareWithScalarOracle) {
    constexpr uint64_t kAttempt = 1'000ull;
    for (uint64_t atmp = 0; atmp < kAttempt; ++atmp) {
        Refresh(atmp);
        float cont_scores[ann::kBatchSize] = {};
        float disc_scores[ann::kBatchSize] = {};
        Compute(cont_scores, disc_scores);
        GradeAgainstScalar(cont_scores);
        GradeAgainstScalar(disc_scores);
    }
}
...
TEST_F(DotProductBatchTest, IdenticalVectors) {
    ...
    for (uint32_t b = 0; b < ann::kBatchSize; ++b) {
        std::copy_n(
            query_, inference::kVectorDim,
            node_batch_ + b * inference::kVectorDim
        );
    }
...
TEST_F(DotProductBatchTest, OppositeVectors) {
    ...
    for (uint32_t b = 0; b < ann::kBatchSize; ++b) {
        EXPECT_NEAR(cont_scores[b], kExpectedDotProduct, kApprox4)
            << "lane=" << b;
...
TEST_F(DotProductBatchTest, MixedLanes) {
    ...
    // Lane 0: Identical.
    std::copy_n(query_, inference::kVectorDim, node_batch_);

    // Lane 1: Opposite.
    for (size_t i = 0; i < inference::kVectorDim; ++i) {
        node_batch_[inference::kVectorDim + i] = -query_[i];
    }

    // Lane 2: Orthogonal.
    {
        float* v2 = node_batch_ + 2 * inference::kVectorDim;
    ...
    float exp_scores[ann::kBatchSize] = {
        1.0f, -1.0f, 0.0f,
        DotProduct(query_, node_batch_ + 3 * inference::kVectorDim)
    };
    float cont_scores[ann::kBatchSize] = {};
    float disc_scores[ann::kBatchSize] = {};
    Compute(cont_scores, disc_scores);
    ...
    for (uint32_t b = 0; b < ann::kBatchSize; ++b) {
        const float expected = exp_scores[b];
        EXPECT_NEAR(cont_scores[b], expected, kApprox4)
            << "lane=" << b << "(" << kLaneNames[b] << ")";
