// Hazard offsets: Hazard pointers but 4 payload byte ranges.
...
// "Hazard zone" structure: `[offset, offset + length)`.
// Used by payload reader to establish protection against GC eviction.
struct HazardRange {
    uint64_t offset;
    uint32_t length;
};

// Hazard zones managing table.
...
private:
    ...
    // Tries unpacking a 64-bit word into a `HazardRange`.
    static std::optional<HazardRange> UnpackHazard(uint64_t word) noexcept {
        ...
        return HazardRange{
            /*offset=*/word & kVirtualOffsetMask,
