// Correctness tests for tagged Treiber Stack.
//
...
//
// Run under ThreadSanitizer to catch specific data race cases.
...
TEST(TreiberStackConcurrency, ConcurrentPushAndPopMaintainsInvariant) {
    ...
    // `state == 0` -> Item currently sits free inside the stack.
    // `state == 1` -> Item is being checked out by any thread.
