// Author: namnkahn1607
//
// RAII wrapper for 32-byte aligned float arrays.
// Required by AVX2 intrinsics: _mm256_load_ps enforces 32-byte alignment
// and will fault on unaligned addresses.

#pragma once

#include <cassert>
#include <cstdlib>
#include <memory>
#include <new>

// AlignedFree: custom deleter for memory allocated by `std::aligned_alloc()`.
// Correct deallocation uses `std::free()`.
struct AlignedFree {
    void operator()(void* ptr) const noexcept {
        std::free(ptr);
    }
};

// AlignedVec: sole owner of a 32-byte aligned float array.
// Destroyed (and memory freed) when the owning scope exits due to RAII.
using AlignedVec = std::unique_ptr<float[], AlignedFree>;

// CreateAlignedVector()
//
// Allocates a 32-byte aligned buffer for `dim` floats and returns it as
// an `AlignedVec`. Throws `std::bad_alloc` on allocation failure.
//
// Precondition: `dim` must be a multiple of 8.
//   An AVX2 register holds 8 floats (256 bits). A dimension that is not a
//   multiple of 8 leaves a partial register unfilled and causes
//   out-of-bounds reads in `DotProductBatch()`.
inline AlignedVec CreateAlignedVector(const size_t dim) {
    assert(dim % 8 == 0 &&
           "dimension must be a multiple of 8 for AVX2 alignment");

    void* ptr = std::aligned_alloc(32, dim * sizeof(float));
    if (ptr == nullptr) {
        throw std::bad_alloc();
    }

    return AlignedVec{static_cast<float*>(ptr)};
}
